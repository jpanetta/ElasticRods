...
sys.path.append(script_dir + '/../3rdparty/knitro')
...
bndsLo = np.array ([ -KTR_INFBOUND, -KTR_INFBOUND ])
bndsUp = np.array ([ 0.5, KTR_INFBOUND ])
m = 2
cType = np.array ([ KTR_CONTYPE_QUADRATIC, KTR_CONTYPE_QUADRATIC ], np.int64)
cBndsLo = np.array ([ 1.0, 0.0 ])
cBndsUp = np.array ([ KTR_INFBOUND, KTR_INFBOUND ])
jacIxConstr = np.array ([ 0, 0, 1, 1 ], np.int32)
jacIxVar    = np.array ([ 0, 1, 0, 1 ], np.int32)
hessRow = np.array ([ 0, 0, 1 ], np.int32)
hessCol = np.array ([ 0, 1, 1 ], np.int32)
